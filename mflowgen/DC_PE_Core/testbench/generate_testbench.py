...
import csv
import re
from defines import scope
import subprocess

def generate_raw(pe_inst, tile, inputs, outputs):
    sv = open('waveform_to_csv.sh', 'w')

    waveform = 'waves'
    clock_period = float(os.environ.get("clock_period"))

    input_signals = ' '.join([f'-signal {scope}.{tile}.PE_inst0.{pe_inst}.{i}' for i in inputs])
    output_signals = ' '.join([f'-signal {scope}.{tile}.PE_inst0.{pe_inst}.{o}' for o in outputs])
    flags = [
        "-overwrite",
        "-xsub 0",
        "-timeunits ps",
    ...
    sv.write(f'simvisdbutil inputs/{waveform}.shm {input_signals} -signal {scope}.{tile}.stall -output raw_input.csv {flag_string}\n')
...
def get_inputs_outputs(design):
    ...
    line = rtl.readline()
    while line:
        if line.startswith(f'module {design}'):
            line = rtl.readline()
            while ');' not in line:
...
def main():
    ...
    pe_inst = 'WrappedPE_inst0'
    if design == 'PE':
        pe_inst += '\$PE_inst0'

    f = open(f'inputs/tiles_Tile_PE.list', 'r')
    i = 0
    for line in f:
        ...
        inputs, outputs = get_inputs_outputs(design)

        generate_raw(pe_inst, tile, inputs, outputs) 
        num_test_vectors, input_widths = convert_raw(inputs+['stall'], "raw_input.csv", f"outputs/tile_tbs/{tile}/test_vectors.txt")
